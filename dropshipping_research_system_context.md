...
- підготовлено два вузли гілки Shopping: нормалізація оферів із протягуванням
  контексту (`niche_id`, `niche`, `keyword_id`, `product_id`) та агрегація
  за рівнями ключа для апдейту `06_Keywords` (Append or Update за
  `keyword_id`, мапиться лише `product_id`, трендові поля не чіпаються).
