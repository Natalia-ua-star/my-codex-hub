...
- підготовлено три вузли гілки Shopping: нормалізація оферів із протягуванням
  контексту (`niche_id`, `niche`, `keyword_id`); агрегація за рівнями ключа
  для апдейту `06_Keywords` (Append or Update за `keyword_id`, мапиться лише
  ...
  (`PRD-{timestamp}`, один на запуск); якщо групові рядки `06_Keywords` уже
